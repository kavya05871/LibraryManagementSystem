...
class Borrower extends User {
    ...
    public void deductDeposit(double amount) { deposit -= amount; }
}
...
class BookService {
    public static void loadBooksFromFile(String filePath, List<Book> books) {
    ...
    public static void saveBooksToFile(String filePath, List<Book> books) {
        try (PrintWriter writer = new PrintWriter(new FileWriter(filePath))) {
            for (Book book : books) {
                writer.printf("%s,%s,%s,%d,%.2f%n", book.getId(), book.getTitle(),
    ...
    public static void addBook(List<Book> books, Scanner sc) {
        ...
        books.add(new Book(id, title, author, qty, cost));
...
static void editOrDeleteBook(List<Book> books, Scanner sc) {
        System.out.print("Enter ID of book to edit/delete: ");
        String id = sc.nextLine();
        Book target = null;
        for (Book b : books) if (b.getId().equalsIgnoreCase(id)) target = b;
        if (target == null) { System.out.println("Book not found."); return; }

        System.out.println("1. Edit\n2. Delete");
        int opt = Integer.parseInt(sc.nextLine());
        if (opt == 1) {
            System.out.print("New Title: "); target.title = sc.nextLine();
            System.out.print("New Author: "); target.author = sc.nextLine();
            System.out.print("New Qty: "); target.quantity = Integer.parseInt(sc.nextLine());
            System.out.print("New Cost: "); target.cost = Double.parseDouble(sc.nextLine());
    ...
    public static void searchBook(List<Book> books, Scanner sc) {
        ...
        for (Book b : books) {
            if (b.getId().equalsIgnoreCase(query) || b.getTitle().toLowerCase().contains(query)) {
...
class BorrowService {
    public static void borrowBook(Borrower b, List<Book> books, Scanner sc) {
    ...
    public static void returnBook(Borrower borrower, List<Book> books, Scanner sc) {
        ...
        for (Book b : books) {
            if (b.getId().equals(returning.getId())) {
...
public class MainApp{
    ...
    public static void main(String[] args) {
        BookService.loadBooksFromFile("books.txt", books);
        ...
        while (true) {
            ...
            if (user instanceof Admin admin) showAdminMenu(admin);
            else if (user instanceof Borrower b) showBorrowerMenu(b);
    ...
    static void showAdminMenu(Admin a) {
        while (true) {
            ...
            switch (ch) {
                case 1 -> BookService.addBook(books, sc);
                case 2 -> BookService.viewBooks(books);
                case 3 -> BookService.searchBook(books, sc);
                case 4 -> BookService.editOrDeleteBook(books,sc);
    ...
    static void showBorrowerMenu(Borrower b) {
        while (true) {
            ...
            switch (ch) {
                ...
                case 2 -> BorrowService.borrowBook(b, books, sc);
                case 3 -> BorrowService.returnBook(b, books, sc);
